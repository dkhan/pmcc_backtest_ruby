...
# Overall judgment
# This run demonstrates that the underlying call-spread concept can generate very large convex gains, but it does not validate the contract-ratchet implementation.
# The reported 67.95% CAGR is compromised by:
# 1. An -82.9% maximum drawdown.
# 2. Contract quantity growing from 7 to 1,007.
# 3. A nominal 0.7% sizing rule that reached 7.3% per entry.
# 4. QuantConnect reporting zero executable strategy capacity.
# 5. Nineteen spreads reaching automatic exercise.
# 6. Twenty-three internal positions left open.
# 7. Four genuinely live spreads at termination.
# 8. $12.3 million of unrealized ending P/L.
# 9. No liquidation on the actual final date.
# 10. Extreme dependence on a handful of winners.
# I would not use this run to choose a real allocation. The useful lesson is that the non-decreasing contract floor is dangerous. The next valid QuantConnect run should use fixed contracts, correct the final-liquidation date, verify zero exercises and zero open spreads, and then test the newer debit/return filters.
