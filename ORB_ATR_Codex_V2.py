...
class ExactQqqOrbPaperReplication(QCAlgorithm):
    ...
    EXPOSURE_CAP = 4.0
    TARGET_R = 10.0
    USE_ATR_VARIANT = True
    ATR_PERIOD = 14
    ATR_STOP_FRACTION = 0.05
    ...
    def Initialize(self):
        ...
        # Figure 8 variant: 5% of the prior 14-day daily ATR. Warm the daily
        # indicator with pre-start history so trading can begin on the first
        # eligible session without using any current-day information.
        self.daily_atr = self.ATR(
            self.symbol,
            self.ATR_PERIOD,
            MovingAverageType.Simple,
            Resolution.Daily,
        )
        self.WarmUpIndicator(self.symbol, self.daily_atr, Resolution.Daily)
        ...
        self.planned_stop_distance = None
        ...
        self.Debug(
            f"ORB ATR VARIANT | {self.TICKER} | 2016-01-01..2023-02-17 | "
            "stop=5% of prior 14-day daily ATR | no profit target | "
            "risk=1% | exposure_cap=4x | fee=$0.0005/share | slippage=0"
        )
    ...
    def OnFiveMinuteBar(self, sender, bar):
        ...
        if self.USE_ATR_VARIANT:
            if not self.daily_atr.IsReady:
                self.rejected_entry_count += 1
                self.traded_today = True
                return
            self.planned_stop_distance = (
                float(self.daily_atr.Current.Value) * self.ATR_STOP_FRACTION
            )
            self.planned_stop = round(
                entry_estimate - direction * self.planned_stop_distance, 2
            )
            risk_per_share = self.planned_stop_distance
        else:
            self.planned_stop = round(
                float(bar.Low if direction > 0 else bar.High), 2
            )
            self.planned_stop_distance = abs(
                entry_estimate - self.planned_stop
            )
            risk_per_share = self.planned_stop_distance
        if risk_per_share <= 0:
            self.rejected_entry_count += 1
    ...
    def HandleEntryFill(self, orderEvent):
        ...
        if self.USE_ATR_VARIANT:
            self.planned_stop = round(
                self.entry_fill_price - direction * self.planned_stop_distance,
                2,
            )
        valid_stop_side = (
        ...
        if not self.USE_ATR_VARIANT:
            target = round(
                self.entry_fill_price +
                direction * self.TARGET_R * risk_per_share,
                2,
            )
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.Log(
            "ORB ATR VARIANT FINAL | "
            f"trades={total} (paper sessions~1795) | "
            f"long={self.long_count} ({long_share:.2%}, paper~51%) | "
            f"short={self.short_count} ({short_share:.2%}, paper~49%) | "
            f"wins={self.win_count} | win_rate={win_rate:.2%} | "
            f"avg_R={average_r:.4f}"
        )
        self.Log(
            "ORB ATR VARIANT EXITS | "
        ...
        self.Log(
            "ORB ATR VARIANT RECONCILIATION | "
        ...
        self.Log(
            "ORB ATR VARIANT PERFORMANCE | "
            f"net_profit={summary['Net Profit']} (paper~9350%) | "
            f"cagr={summary['Compounding Annual Return']} | "
            f"sharpe={summary['Sharpe Ratio']} | "
